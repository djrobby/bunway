<script lang="ts">
  ...
  let records = $state<Tag[]>([])
  let form = $state({ name: '' })
  let editing = $state<Tag['id'] | null>(null)
  let message = $state('')
  let total = $state(0)
  let page = $state(1)
  ...
  async function load() {
    ...
    if (error) message = 'Could not load tags'
    else {
  ...
  async function save(event: SubmitEvent) {
    ...
    const result =
      editing === null ? await api.tags.post(form) : await api.tags({ id: editing }).patch(form)
    if (result.error) message = 'Could not save tag'
    else {
      cancel()
      await load()
    }
  ...
  async function remove(id: Tag['id']) {
    const { error } = await api.tags({ id }).delete()
    if (error) message = 'Could not delete tag'
    else await load()
  }
  ...
  function toggleRow(id: Tag['id'], checked: boolean) {
    selectedIds = checked
...
<main class="w-full min-w-0 px-4 py-8 sm:px-6 lg:px-8">
  <div class="flex items-end justify-between gap-4">
    ...
    <Button type="button" onclick={openNew}>New Tag</Button>
  </div>
  <Dialog.Root bind:open={dialogOpen}>
    <Dialog.Content class="sm:max-w-xl">
      <Dialog.Header>
        <Dialog.Title>{editing === null ? 'New' : 'Edit'} Tag</Dialog.Title>
        <Dialog.Description>Enter the tag details below.</Dialog.Description>
      </Dialog.Header>
    ...
    <Table.Root class="w-max min-w-full table-fixed">
      <Table.Header>
        <Table.Row>
          ...
          {#if visible.name}
            <ResizableHead resource="tags" column="name" class="px-3 py-2 text-left">
              <button
